...
def extract_conversations(chat_db, threshold):
    ...
    if chat_db[0]["sender"] != chat_db[1]["sender"]:
        update_followup_tab(
            conversations[-1]["followup_tab"], chat_db, 0, users, threshold
        )
    for i in range(1, len(chat_db)):

        if i % 721 == 0:
            print(
                "Splitting chat into conversations, at %d %%"
                % int(i * 100 / len(chat_db)),
                end="\r",
            )
        if chat_db[i]["posix"] - chat_db[i - 1]["posix"] > threshold:
            ...
            conversations[-1]["n_chars"][chat_db[i]["sender"]] += len(
                chat_db[i]["message"]
            )
            if chat_db[i]["type_id"] != 0:
                conversations[-1]["n_media"][chat_db[i]["sender"]] += 1
            if i < len(chat_db) - 1:
                if chat_db[i]["sender"] != chat_db[i + 1]["sender"]:
                    update_followup_tab(
                        conversations[-1]["followup_tab"], chat_db, i, users, threshold
                    )
...
def update_followup_tab(followup_tab, chat_db, idx, users, threshold):
    ...
    for u in users:
        if u != sender:
            followup_tab[sender][u].append(
                min_followup(u, chat_db, idx, threshold, max_idx)
            )
    return
...
def flatten_followup(conversations):
    ...
    for k1 in followup_tab:
        for k2 in followup_tab[k1]:
            followup_tab[k1][k2] = list(
                filter(lambda x: x != float("Inf"), followup_tab[k1][k2])
            )
    for k1 in followup_tab:
        for k2 in followup_tab[k1]:
