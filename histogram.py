...
def time_histograms(chat_db, settings):
    ...
    plt.xticks( np.arange(1.5, 13.5, 1.0), \
    ...
    plt.xticks( np.linspace(0.5, 5.5, 7), \
...
def text_histograms(chat_db, settings):
    ...
    for sender in text_length_distr:
        for i in range(0, len(text_length_distr[sender])):
            if text_length_distr[sender][i] > max_per:
                text_length_distr[sender][i] = max_per + log(text_length_distr[sender][i]-max_per+1, log_base)
    ...
    for sender in text_length_distr:
        fig, ax = plt.subplots()
        ax.hist(text_length_distr[sender], bins=nbins, edgecolor='black', log=True, range=(0, max_len))
        plt.xticks( np.concatenate((np.arange(0, max_per, 50), np.arange(max_per, max_len, 40))), \
                    np.concatenate((np.arange(0, max_per, 50), log_base**(np.arange(max_per+40, max_len, 40)-max_per)+max_per-1)).astype(int) )
        plt.ylim((0.7, ylim))
        fig.savefig(settings["savepath"]+sender.replace(" ","")+"_texts_len_distr.png", dpi=settings["dpi"])
...
def tgap_histogram(chat_db, settings):
    ...
    for i in range(0, len(chat_db)-1):
        ...
        if (chat_db[i]["sender"] != chat_db[i+1]["sender"] or
            chat_db[i+1]["posix"] - chat_db[i]["posix"] > tgap_lim):
            distribution.append(chat_db[i+1]["posix"] - chat_db[i]["posix"] + 1)

    fig, ax = plt.subplots()
    ax.hist(np.log10(distribution), bins=40, edgecolor='black')
    plt.xticks( np.log10([1, 10, 60, 600, 3600, 5*3600, 86400, 7*86400, 30*86400]), \
