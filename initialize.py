...
def check_settings(settings):
    ...
    for key in ["filename", "yearin2000", "time_re", "plaintext_re", "mediatext_re", \
        "emptytext_re", "nameseparator", "contacts", "mediatype", "dpi", \
        "savepath", "csv_separator"]:
        if key not in settings:
            print("WARNING, 'config.json' does not contain", key)

    for key in ["?P<year>", "?P<month>", "?P<day>", "?P<hour>", "?P<minute>", "?P<second>"]:
